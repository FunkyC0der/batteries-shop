...
- [ ] **Реальні контакти.** У `src/lib/site-config.ts` телефон і месенджери
      ...
      `contactsPublished: true`. Залишилось: місто, адреса, графік роботи —
      досі заглушки (`city`, `address`, `schedule`), замінити на реальні перед
      запуском реклами.
- [ ] **Фінальний домен.** Виставити `NEXT_PUBLIC_SITE_URL` у Vercel
      (Production). Рекламу й Search Console вести на нього, не на
      `*.vercel.app`.
- [ ] **Реквізити продавця** (ФОП/юрособа: назва, ЄДРПОУ/ІПН, адреса).
      Розмітка у футері й на `/privacy/` вже готова
      (`siteConfig.seller.{name,taxId,address}`) — треба замінити заглушки на
      реальні дані з витягу ЄДР перед запуском реклами.
