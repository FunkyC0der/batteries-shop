...
- **Реальні контакти.** Зараз у `siteConfig` плейсхолдери
  (`+380000000000`, `placeholder_batteries`). Реклама на сайт з
  непрацюючими контактами буде відхилена (політика Google Ads щодо
  неправдивого представлення / вимоги до цільової сторінки), а всі
  конверсії вестимуть «в нікуди». Замінити й виставити
  `contactsPublished: true`.
- **Реквізити продавця.** Для політики конфіденційності та довіри Google
  бажано вказати юрособу/ФОП (назва, ЄДРПОУ/ІПН, адреса) — додати на
  `/privacy/` і у футер, коли дані будуть.
- **Доставка, оплата, повернення.** Для інтернет-магазину Google очікує
  зрозумілі умови (окрема сторінка). Потрібні реальні умови — не вигадувати.
- **Домен.** Рекламу запускати на фінальному домені (`NEXT_PUBLIC_SITE_URL`),
  не на `*.vercel.app`.

## Поза поточним обсягом

- Банер згоди (CMP) — лише якщо реклама таргетуватиметься на ЄЕЗ/UK.
- Google Merchant Center / Shopping-реклама — потребує фіду товарів із цінами
  (частина товарів без ціни) та сторінки умов повернення.
- Дзвінки як конверсії з номерами переадресації (Google forwarding numbers).
